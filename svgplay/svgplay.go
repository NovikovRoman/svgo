...
func FrontPage(w http.ResponseWriter, req *http.Request) {
	...
	_ = frontPage.Execute(w, data) //! ignore error
}
...
func Compile(w http.ResponseWriter, req *http.Request) {
	...
	_, _ = w.Write(out) //! ignore error
}

var (
	tmpdir, buildpid string
)
...
func compile(req *http.Request) (out []byte, err error) {
	...
	defer os.Remove(src)
	if err = os.WriteFile(src, body.Bytes(), 0666); err != nil {
		return
	}
...
func compileError(w http.ResponseWriter, out []byte, err error) {
	...
	if out != nil {
		...
		for _, l := range elines {
			...
			_ = output.Execute(w, l[i+1:]) //! ignore error
		}
	} else {
		_ = output.Execute(w, err.Error()) //! ignore error
	}
}
...
import (
	...
	"os"
	"time"
	"github.com/NovikovRoman/svgo/float"
)
